...
function config_url {
    ...
    echo "https://dl.cloudsmith.io/public/infisical/infisical-cli/config.alpine.txt?$params" | sed 's/ /%20/g'
}
...
EOF
 ...
 >>>>: doesn't work after an hour, please contact Infisical
 >>>>: for assistance.
 >>>>:
EOF
        }
...
function import_rsa_key {
    local text="Importing 'infisical/infisical-cli' repository RSA key..."
    echo_running "$text"
...
function setup_repository {
    ...
    local text="Installing 'infisical/infisical-cli' repository..."
    echo_running "$text"
...
echo "Executing the  setup script for the 'infisical/infisical-cli' repository ..."
echo
